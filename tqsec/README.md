...
Thin instrumentation and control layer over the vendored TurboQuant
(`third_party/turboquant/`). Nothing here reimplements the algorithm.
...
| `quantizers.py` | **done** | One interface over `{TurboQuant, INT, KIVI, FP8}` + the `-nc` policy + bit accounting; device-safe; records to an error map for cross-codec comparison. Smoke: `scripts/quantizers_smoke.py`. Finding: TurboQuant's error geometry (channel-decorrelated, inner-product-optimal) is distinguishable from INT/KIVI/FP8. |
...
| `inversion.py` | **done** | Learned token-inverter helper for T3. Trains a compact classifier on FP or compressed key vectors and reports token recovery separately from semantic recovery. Smoke: `scripts/t3_leakage_smoke.py`. |
