...
The single biggest security variable (PROJECT_PLAN.md §3.3). TurboQuant reuses a
pre-generated Π for speed — but a fixed, attacker-known Π is exactly what makes both
attacks tractable (you can de-rotate; you can solve against the geometry). A fresh,
secret Π per deployment is a strong barrier. Both attacks are reported under both
regimes, and how Π-secrecy moves attack success is itself a contribution.

This module is deliberately thin: a Π regime is really just *seed management* plus a
statement of **what the attacker is allowed to know**.
...
`attacker_seed()` is the crux: public → the real seed (attacker can rebuild Π);
secret → None (attacker must attack without Π). Feed `seed_for_layer` to
