...
analysis are landed. LongBench/perplexity and measured serving memory remain future work (section 7).
...
5.0×) all scored ≤ 0.17 on Qwen and were run for the section 5 mechanism analysis only.
...
survive. **`kivi3` gives the highest compression *and* works on every model.** It is the robust memory×quality
choice; TurboQuant must fall back to `-nc` (2.9×, lower compression) to function
...
- **Values are well-behaved:** max|V| = 72.5, worst channel only 4.0× median, so **no value outliers.**
...
channels (worst ≤ 6× median, section 4), so this failure mode does not arise for them and everything below is
...
- **Quality = needle retrieval + perplexity** (section 3), across three models. A LongBench slice was tried but
...
- **Counted, not measured, memory** (section 3). The figures are bytes-stored (codes + scales + QJL); a
  *measured* number under a serving allocator needs vLLM (deferred).
