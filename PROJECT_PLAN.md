...
**Status:** Phase 0 (reuse audit), Phase 1 (instrumented foundation), T1 characterization, and runnable T2/T3 harnesses are done. T2/T3 result claims are pending Leonardo runs. (Greenfield restart of `../turboquant-kv-cache-experiments`.)
...
- **Deliverable:** `reports/T2_behavior.md` — operational definition of the canary behavior (a *specific target*, explicitly **not** output drift), FP-vs-TurboQuant contrast on the *same* behavior, the INT/KIVI ablation, Π-regime results.
